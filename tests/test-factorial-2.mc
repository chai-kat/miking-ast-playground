...
let imperative_ast = funcdecl_ 
    [
        (stmtmatch_ (eqi_ (var_ "n") (int_ 0)) ptrue_
            [(return_ (int_ 1))]
            []
        ),
        (nvardecl_ (nameNoSym "result") tyint_ (int_ 1)),
        (while_ (neqi_ (var_ "n") (int_ 1)) [
            (varassign_ (nameNoSym "result") (muli_ (var_ "result") (var_ "n"))), -- result = result*x
            (varassign_ (nameNoSym "n") (subi_ (var_ "n") (int_ 1)))  -- x = x-1
        ]),
        return_ (var_ "result")
    ]
    ...
    [
        param_ (nameNoSym "n") tyint_
    ]
 in

-- let fullprogram = bind_ imperative_ast (appf1_ "printLn" (appf1_ "int2string" (appf1_ "fact" (int_ 5)))) in
